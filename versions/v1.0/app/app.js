...
function renderCards() {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    slot.disabled = state.phase !== "HOLD_SELECT";
...
function renderStatus() {
  ...
  if (state.phase === "HOLD_SELECT") {
    el.dealDrawBtn.textContent = "Draw (Enter)";
  } else if (state.phase === "RESULT") {
    el.dealDrawBtn.textContent = "Next Hand (Enter)";
...
function wireEvents() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (state.phase === "HOLD_SELECT" && /^[1-5]$/.test(key)) {
      e.preventDefault();
